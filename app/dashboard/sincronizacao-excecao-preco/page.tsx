...
import { RefreshCw, Clock, Database, DollarSign, Settings } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
    try {
      ...
      const response = await fetch(
        `/api/sync/excecao-preco?idSistema=${idSistema}&empresa=${encodeURIComponent(empresa)}`,
        { method: 'POST' }
      )
  ...
  const handleAutoSyncToggle = (idSistema: number, enabled: boolean) => {
    ...
    if (enabled) {
      ...
      toast({
        ...
        description: `Exceções de preço de ${contrato?.EMPRESA} serão sincronizadas a cada ${config.intervalMinutes} minutos`,
      })
    } else {
      if (config.intervalId) {
        clearInterval(config.intervalId)
      ...
      toast({
        ...
        description: `Exceções de preço de ${contrato?.EMPRESA} não serão mais sincronizadas automaticamente`,
      })
    }
  }
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de exceções de preço entre Sankhya e o sistema local
            </p>
          </div>
          <Button
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sincroniza automaticamente as exceções de preço desta empresa
                    </p>
                  </div>
                  <Switch
                    id="auto-sync-toggle"
