...
import { RefreshCw, Clock, Database, Users, Settings } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
    try {
      ...
      const response = await fetch(
        `/api/sync/vendedores?idSistema=${idSistema}&empresa=${encodeURIComponent(empresa)}`,
        { method: 'POST' }
      )
  ...
  const handleAutoSyncToggle = (idSistema: number, enabled: boolean) => {
    ...
    if (enabled) {
      ...
      toast({
        ...
        description: `Vendedores de ${contrato?.EMPRESA} serão sincronizados a cada ${config.intervalMinutes} minutos`,
      })
    } else {
      if (config.intervalId) {
        clearInterval(config.intervalId)
      ...
      toast({
        ...
        description: `Vendedores de ${contrato?.EMPRESA} não serão mais sincronizados automaticamente`,
      })
    }
  }
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de vendedores e gerentes entre Sankhya e o sistema local
            </p>
          </div>
          <Button
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sincroniza automaticamente os vendedores desta empresa
                    </p>
                  </div>
                  <Switch
                    id="auto-sync-toggle"
