  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
    try {
      ...
      const response = await fetch(
        `/api/sync/financeiro?idSistema=${idSistema}&empresa=${encodeURIComponent(empresa)}`,
        { method: 'POST' }
      )
  ...
  const handleAutoSyncToggle = (idSistema: number, enabled: boolean) => {
    ...
    if (enabled) {
      ...
      toast({
        ...
        description: `Financeiro de ${contrato?.EMPRESA} será sincronizado a cada ${config.intervalMinutes} minutos`,
      })
    } else {
      if (config.intervalId) {
        clearInterval(config.intervalId)
      ...
      toast({
        ...
        description: `Financeiro de ${contrato?.EMPRESA} não será mais sincronizado automaticamente`,
      })
    }
  }
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sincroniza automaticamente o financeiro desta empresa
                    </p>
                  </div>
                  <Switch
                    id="auto-sync-toggle"
