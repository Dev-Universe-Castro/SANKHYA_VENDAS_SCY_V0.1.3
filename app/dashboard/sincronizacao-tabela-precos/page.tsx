  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
    try {
      ...
      const response = await fetch(
        `/api/sync/tabela-precos?idSistema=${idSistema}&empresa=${encodeURIComponent(empresa)}`,
        { method: 'POST' }
      )
  ...
  const handleAutoSyncToggle = (idSistema: number, enabled: boolean) => {
    ...
    if (enabled) {
      ...
      toast({
        ...
        description: `Tabelas de preços de ${contrato?.EMPRESA} serão sincronizadas a cada ${config.intervalMinutes} minutos`,
      })
    } else {
      if (config.intervalId) {
        clearInterval(config.intervalId)
      ...
      toast({
        ...
        description: `Tabelas de preços de ${contrato?.EMPRESA} não serão mais sincronizadas automaticamente`,
      })
    }
  }
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de tabelas de preços entre Sankhya e o sistema local
            </p>
          </div>
          <Button
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sincroniza automaticamente as tabelas de preços desta empresa
                    </p>
                  </div>
                  <Switch
                    id="auto-sync-toggle"
