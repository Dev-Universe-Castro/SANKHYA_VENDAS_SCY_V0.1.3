...
import { RefreshCw, Clock, Database, Package } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoEstoquesPage() {
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
    try {
      ...
      toast({
        title: "Sincronização iniciada",
        description: `Sincronizando estoques de ${empresa}...`
      })

      const response = await fetch(
        `/api/sync/estoques?idSistema=${idSistema}&empresa=${encodeURIComponent(empresa)}`,
        { method: 'POST' }
      )
      ...
      <div className="space-y-6 p-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de estoques entre Sankhya e o sistema local
            </p>
          </div>
          <Button
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Status de sincronização de estoques por empresa
            </CardDescription>
          </CardHeader>
          <CardContent>
            {contratos.length === 0 ? (
