...
import { RefreshCw, Clock, Database, FileText, Settings } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoCabecalhoNotaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const statsRes = await fetch('/api/sync/cabecalho-nota')
      if (statsRes.ok) {
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando cabeçalhos de nota de todas as empresas..."
      })

      const response = await fetch('/api/sync/cabecalho-nota', {
        method: 'POST'
      })
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
    try {
      ...
      toast({
        title: "Sincronização iniciada",
        description: `Sincronizando cabeçalhos de nota de ${empresa}...`
      })

      const response = await fetch(
        `/api/sync/cabecalho-nota?idSistema=${idSistema}&empresa=${encodeURIComponent(empresa)}`,
        { method: 'POST' }
      )
  ...
  const handleAutoSyncToggle = (idSistema: number, enabled: boolean) => {
    ...
    if (enabled) {
      ...
      toast({
        ...
        description: `Cabeçalhos de nota de ${contrato?.EMPRESA} serão sincronizados a cada ${config.intervalMinutes} minutos`,
      })
    } else {
      if (config.intervalId) {
        clearInterval(config.intervalId)
      ...
      toast({
        ...
        description: `Cabeçalhos de nota de ${contrato?.EMPRESA} não serão mais sincronizados automaticamente`,
      })
    }
  }
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold">Sincronização de Cabeçalhos de Nota</h1>
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de cabeçalhos de nota entre Sankhya e o sistema local
            </p>
          </div>
          <Button
            ...
            {selectedEmpresa && (
              <div className="space-y-4 py-4">
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sincroniza automaticamente os cabeçalhos de nota desta empresa
                    </p>
                  </div>
                  <Switch
                    id="auto-sync-toggle"
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={String(autoSyncConfigs.get(selectedEmpresa.ID_EMPRESA)?.intervalMinutes || 360)}
                    onValueChange={(value) =>
