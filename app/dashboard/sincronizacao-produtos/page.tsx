...
import { RefreshCw, Clock, Database, Package, Settings } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function SincronizacaoProdutosPage() {
  ...
  const sincronizarTodas = async () => {
    try {
      ...
      toast({
        ...
        description: "Sincronizando produtos de todas as empresas (uma por vez)..."
      })
  ...
  const sincronizarEmpresa = async (idSistema: number, empresa: string) => {
    try {
      ...
      toast({
        title: "Sincronização iniciada",
        description: `Sincronizando produtos de ${empresa}...`
      })

      const response = await fetch(
        `/api/sync/produtos?idSistema=${idSistema}&empresa=${encodeURIComponent(empresa)}`,
        { method: 'POST' }
      )
  ...
  const handleAutoSyncToggle = (idSistema: number, enabled: boolean) => {
    ...
    if (enabled) {
      ...
      const intervalId = setInterval(() => {
        console.log(`🔄 Executando sincronização automática de produtos para empresa ${idSistema}...`)
      ...
      toast({
        ...
        description: `Produtos de ${contrato?.EMPRESA} serão sincronizados a cada ${config.intervalMinutes} minutos`,
      })
    } else {
      if (config.intervalId) {
        clearInterval(config.intervalId)
      ...
      toast({
        ...
        description: `Produtos de ${contrato?.EMPRESA} não serão mais sincronizados automaticamente`,
      })
    }
  }
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Gerencie a sincronização de produtos entre Sankhya e o sistema local
            </p>
          </div>
          <Button
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Sincroniza automaticamente os produtos desta empresa
                    </p>
                  </div>
                  <Switch
                    id="auto-sync-toggle"
